...
namespace RTEditor
{
    ...
    public class TranslationGizmo : Gizmo
    {
        ...
        protected override void OnInputDeviceMoved()
        {
                ...
                if (IsSurfacePlacementShActive && ControlledObjects != null && _isSpecialOpSquareSelected)
                {
                    if (_transformPivotPoint == TransformPivotPoint.Center)
                    {
                        if (_objectOffsetsFromGizmo.Count == 0)
                        {
                            ...
                            {
                                Box worldAABB = parent.GetWorldBox();
                                if (worldAABB.IsValid()) _objectOffsetsFromGizmo.Add(parent, worldAABB.Center - _gizmoTransform.position);
                                else _objectOffsetsFromGizmo.Add(parent, parent.transform.position - _gizmoTransform.position);
                            }
                        }
                    }
                    else
                            ...
                            if (topParents.Count != 0)
                            {
                                ...
                                foreach (var parent in topParents)
                                {
                                    ...
                                    if (_transformPivotPoint == TransformPivotPoint.Center)
                                    {
                                        ...
                                        {
                                            Vector3 fromCenterToPos = parentTransform.position - worldAABB.Center;
                                            Vector3 newCenter = newGizmoPos + _objectOffsetsFromGizmo[parent];
                                            parentTransform.position = newCenter + fromCenterToPos;
                                        }
                                        else parentTransform.position = newGizmoPos + _objectOffsetsFromGizmo[parent];
                                    }
                                    else parentTransform.position = newGizmoPos + _objectOffsetsFromGizmo[parent];

                                    Ray terrainPickRay = new Ray(newGizmoPos, -Vector3.up);
                        ...
                        if (hitMeshObject != null)
                        {
                            ...
                            if (topParents.Count != 0)
                            {
                                foreach (var parent in topParents)
                                {
                                    ...
                                    Ray meshPickRay = new Ray(newGizmoPos + hitNormal, -hitNormal);
            ...
            // Used to eliminate initial snap of objects when doing surface placement. However, the results are not that great...
            /*else
            {
                if (IsSurfacePlacementShActive)
                {
                    ...
                    if (cursorRayHit.WasAnObjectHit && cursorRayHit.ClosestObjectRayHit.WasTerrainHit)
                    {
                        _gizmoTransform.position = cursorRayHit.ClosestObjectRayHit.HitPoint;
                    }
                    else
                    if (cursorRayHit.WasAnObjectHit && cursorRayHit.ClosestObjectRayHit.WasMeshHit)
                    {
                    ...
                    if (cursorRayHit.WasACellHit)
                    {
                        _gizmoTransform.position = cursorRayHit.GridCellRayHit.HitPoint;
                    }
                }
            }*/
        }
