...
public static class TCP2_Utils
{
	...
	static public float ScreenWidthRetina { get { return Screen.width/EditorGUIUtility.pixelsPerPoint; } }
#else
	...
	static public bool ShaderKeywordRadio(string header, string[] keywords, GUIContent[] labels, List<string> list, ref bool update)
	{
		...
		if(hasHeader)
		{
			...
			r.width = TCP2_Utils.ScreenWidthRetina - headerRect.width - 34f;
			r.x += headerRect.width;
		}
		else
		{
			r.width = TCP2_Utils.ScreenWidthRetina - 34f;
		}
	...
	static public int ShaderKeywordRadioGeneric( string header, int index, GUIContent[] labels )
	{
		//Header and rect calculations
		bool hasHeader = (!string.IsNullOrEmpty(header));
		...
		{
			r.width = TCP2_Utils.ScreenWidthRetina - 20f;
		}
	...
	static public void RemoveIfExists( List<string> list, string item )
	{
		if (list.Contains(item))
			list.Remove(item);
	}
...
//			EditorUtility.CopySerialized(originalMesh, newMesh);
	...
	static public bool SystemToUnityPath(ref string sysPath)
	{
