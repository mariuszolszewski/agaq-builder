...
namespace AgaQ.Bricks
{
    static public class MeshTools
    {
            ...
            for (int i = 0; i < oldVertices.Length; i++)
            {
                ...
                for (int j = 0; j < buckets[x, y, z].Count; j++)
                {
                    ...
                    if (Vector3.SqrMagnitude(to) < 0.001f)
                    {
                        ...
                        continue; // Skip to next old vertex if this one is already there
                    }
                }
        ...
        public static void Simplify(this Mesh mesh)
        {
            ...
            foreach (Vertice v in workingSet)
            {
                if (!v.IsStatic)
        ...
        class Vertice
        {
            ...
            public int finalIndex; //A cache used for identifying a vertice during mesh reconstruction.
            ...
            public Edge GetExistingConnectingEdge(Vertice v)
            {
                ...
                {
                    if ((e.vertices[0] == this && e.vertices[1] == v) || (e.vertices[1] == this && e.vertices[0] == v))
                        return e;
                }
            ...
            /// <summary>
            /// When collapsing an edge a->c, the linear space must
        ...
        class Edge
        {
            ...
            public static void Collapse(Edge moved, Edge target, Face f)
            {
                ...
                try
                {
                    faceOutsideMoved = moved.GetOpposite(f);
                }
                catch (Exception e)
                {
                    throw new Exception(string.Format("{0}\n{1} <--> {2}", e.Message, moved.vertices[0].position, moved.vertices[1].position));
                }
        ...
        class Face
        {
            ...
            /// <summary>
            /// Initializes a new instance of the <see cref="T:AgaQ.Bricks.MeshTools.Face"/> class.
            /// </summary>
            /// <param name="e0">Edge between v0 and v1</param>
            /// <param name="e1">Edge between v0 and v2</param>
            /// <param name="e2">Edge between v1 and v2</param>
            Face(Edge e0, Edge e1, Edge e2)
            {
            ...
            /// Adds the face.
            /// </summary>
            /// <param name="allVertices">List of vertices, in the same order as 'verts' from the mesh.</param>
            /// <param name="tris">The tris array from the mesh.</param>
            /// <param name="triIndex">The index of the first vertex in this triangle.</param>
            ...
            public Edge GetOpposite(Vertice v)
            {
                ...
                if (!(e0 = edges[0]).Contains(v))
                    return e0;

                if (!(e1 = edges[1]).Contains(v))
                    return e1;
            ...
            public bool MoveWouldFlip(Vertice v, Vector3 p)
            {
                ...
                if (c0SqrMagnitude < 0.0001f)
                    return true;

                var cross1 = Vector3.Cross(ct, ov);
                var c1SqrMagnitude = cross1.sqrMagnitude;
                if (c1SqrMagnitude < 0.0001f)
                    return true;

                if (Mathf.Sign(Vector3.Dot(cross0, cross1)) < 0f)
                    return true;
